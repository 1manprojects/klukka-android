...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navView.doOnLayout {
            val insets = ViewCompat.getRootWindowInsets(navView)
            val statusBarTop = insets?.getInsets(WindowInsetsCompat.Type.statusBars())?.top ?: 0
            val basePadding = (24 * resources.displayMetrics.density + 0.5f).toInt()
            navView.getHeaderView(0).updatePadding(top = statusBarTop + basePadding)
        }
